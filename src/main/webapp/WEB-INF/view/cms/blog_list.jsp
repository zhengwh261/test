				...
				<tbody>
					<c:forEach items="${page.list}" var="blog">
						<tr>
							...
							<td><a
								href="
								${pageContext.request.contextPath }/cms/blogContents?id=${blog.id }"
								class="BContent">《 ${blog.title } 》</a></td>
							<td><fmt:formatDate value="${blog.createTime }"
									pattern="yyyy-MM-dd HH:mm:ss" /></td>
							<td>${blog.author }</td>
							<td><a class="	glyphicon glyphicon-remove"
								href="${pageContext.request.contextPath }/cms/delete?id=${blog.id }" class="btn btn-danger" role="button">删除</a>
							<a class="	glyphicon glyphicon-edit"
								   href="${pageContext.request.contextPath }/cms/toupdate?id=${blog.id }" class="btn btn-danger" role="button">编辑</a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
